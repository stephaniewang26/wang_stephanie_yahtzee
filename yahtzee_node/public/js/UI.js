...
let my_username = (document.getElementById("game_heading").textContent).slice(document.getElementById("game_heading").textContent.indexOf(":")+2)
let game_name = (document.getElementById("game_heading").textContent).slice(0,document.getElementById("game_heading").textContent.indexOf(":"))
...
function enter_score_handler(event){
    ...
    console.log(element)
    console.log("event score handler read value "+ value)

    let split_element = element.id.split("|")
    let username = split_element[1]
    console.log(username)

    let category_name = split_element[0].replace("_input","");
    ...
    if (gamecard.is_valid_score(category_name,value) == true){
        ...
        //send socket message w/ to object, which will propagate out to other people and also be sent in a post request to the python server to update scorecard
        console.log("🚀 Emitting valid_score_submitted with:", {
            event_username: username,
            score_info: gamecard.to_object_specific(username)
        });
        socket.emit('valid_score_submitted', {
            event_username: username,
            score_info: gamecard.to_object_specific(username),
            game_name: game_name
        });
        console.log(gamecard.to_object_specific(username))
    }
    else{
